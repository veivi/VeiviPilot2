...
Damper_t accAvg = DAMPER_CONS(4*CONFIG_HZ, 9.81);
Sampler_t alphaSampler = SAMPLER_CONS, iasSampler = SAMPLER_CONS;
SlopeLimiter_t aileActuator = SLOPE_CONS(0), trimRateLimiter = SLOPE_CONS(3/RADIAN), flapActuator = SLOPE_CONS(0.5);
Damper_t iasEntropy, alphaEntropy;
Washout_t yawDamper = WASHOUT_CONS(0.4*CONTROL_HZ, 0);
Turbine_t engine;
