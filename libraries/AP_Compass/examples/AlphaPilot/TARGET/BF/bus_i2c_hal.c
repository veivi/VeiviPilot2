...
bool i2cReadGeneric(I2CDevice device, uint8_t target_, uint8_t addrSize, const uint8_t *addr, uint8_t len, uint8_t* buf)
{
  bool success = true;
    ...
    switch(addrSize) {
    case 0:
      status = HAL_I2C_Master_Receive(pHandle ,target_ << 1, buf, len, I2C_DEFAULT_TIMEOUT);
      break;

    case 1:
      status = HAL_I2C_Mem_Read(pHandle, target_ << 1, addr[0], I2C_MEMADD_SIZE_8BIT,buf, len, I2C_DEFAULT_TIMEOUT);
      break;

    case 2:
      status = HAL_I2C_Mem_Read(pHandle, target_ << 1, (addr[1]<<8) + addr[0], I2C_MEMADD_SIZE_16BIT,buf, len, I2C_DEFAULT_TIMEOUT);
      break;
