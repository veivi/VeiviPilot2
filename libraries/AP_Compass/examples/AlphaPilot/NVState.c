...
bool readNVState(void)
{
  if(!m24xxRead(stateOffset, (uint8_t*) &nvState, sizeof(nvState))) {
    consoleNoteLn_P(CS_STRING("NV storage NOT PRESENT"));
    return false;
  }
...
void printParams()
{
  ...
  consolePrintF(vpParam.vertDefl*90.0f/500);
  consolePrint(", ");
  consolePrintLnF(vpParam.horizDefl*90.0f/500);
  consoleNote_P(CS_STRING("    Rudder      "));
  consolePrintF(vpParam.rudderDefl*90.0f/500);
  consoleTab(30);
  consolePrint_P(CS_STRING("aile mix = "));
  consolePrintLnF(vpParam.r_Mix);
  consoleNote_P(CS_STRING("    Nose wheel  "));
  consolePrintF(vpParam.steerDefl*90.0f/500);
  consoleTab(30);
  consolePrint_P(CS_STRING("trim = "));
  consolePrintLnF(vpParam.steerTrim*90);
  consoleNote_P(CS_STRING("    Flap        "));
  consolePrintLnF(vpParam.flapDefl*90.0f/500);
  consoleNote_P(CS_STRING("    Brake       "));
  consolePrintLnF(vpParam.brakeDefl*90.0f/500);
